...
from xnup.exceptions import TelegramInvalidFile
from xnup.files import get_file_attributes, RecursiveFiles, NoDirectoriesFiles, MAX_FILE_SIZE, \
...
class TestGetFileAttributes(unittest.TestCase):
    ...
    @patch('xnup.files.video_metadata')
...
class TestRecursiveFiles(unittest.TestCase):
    @patch('xnup.files.scantree', return_value=[])
    @patch('xnup.files.os.path.isdir', return_value=False)
    def test_one_file(self, m1, m2):
        self.assertEqual(list(RecursiveFiles(['foo'])), ['foo'])

    @patch('xnup.files.scantree')
    @patch('xnup.files.os.path.isdir', return_value=True)
...
class TestNoLargeFiles(unittest.TestCase):
    @patch('xnup.files.os.path.getsize', return_value=MAX_FILE_SIZE - 1)
    @patch('xnup.files.File')
...
class TestSplitFiles(unittest.TestCase):
    ...
    @patch('xnup.files.os.path.getsize', return_value=MAX_FILE_SIZE + 1000)
    @patch('xnup.files.SplitFile.__init__', return_value=None)
    @patch('xnup.files.SplitFile.seek')
